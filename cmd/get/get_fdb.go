...
func NewGetFDBCmd(restOptions *api.RESTOptions) *cobra.Command {
	var GetFDBCmd = &cobra.Command{
		Use:   "fdb",
		Short: "Get a fdb",
		Long:  `It shows fdb Information in the LoxiLB`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err != nil {
				return exitcode.Unavailablef("get fdb: %v", err)
			}
			if resp.StatusCode == http.StatusOK {
				PrintGetFDBResult(resp, *restOptions)
				return nil
			}
			return exitcode.FromHTTPStatus("get fdb", resp.StatusCode)
		},
	}
...
func PrintGetFDBResult(resp *http.Response, o api.RESTOptions) {
	FDBresp := api.FDBModGet{}
	var data [][]string
